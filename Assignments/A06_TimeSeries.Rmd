...
# Initial Eno River Discharge over Time Plot
```{r, fig.width=10}
library(scales)
EnoRiverPlot <- 
  ggplot(EnoRiverDischarge, aes(x = Date)) +
  geom_line(aes(y=Discharge, color="#cd5555"), show.legend=FALSE) +
...
```
>Data gap between 10/21/2017 and 11/27/2017. Time series objects requires equispaced data. 

## Remove data gap time period
```{r}
EnoRiverDischargeNoGap<-filter(EnoRiverDischarge, Date<"2017-10-21")
```

# Replot Eno River Discharge Data w/o Gap
```{r, fig.width=13}
library(scales)
EnoRiverPlotNoGap <- 
  ggplot(EnoRiverDischargeNoGap, aes(x = Date, y = Discharge)) +
  geom_line(aes(y=Discharge, color="#cd5555"), show.legend=FALSE) +
  labs(title="Eno River Discharge over Time", x = "Year", y = expression("Discharge (ft"^3*"/s)")) + 
  ...
  scale_y_continuous(breaks=seq(0,10000, by = 2000))+
   scale_x_date(labels = date_format("%Y"), breaks = date_breaks("5 years"))+
  gabytheme
print(EnoRiverPlotNoGap)
```


## Create Discharge Time Series
```{r}
EnoRiver_ts <- ts(EnoRiverDischargeNoGap[[4]], frequency = 365) #select 4th column-discharge
```
...
```{r}
EnoRiver_Components <- as.data.frame(EnoRiver_Decomposed$time.series[,1:3]) ###Pull in the timeseries list to create a dataframe


EnoRiver_Components <- mutate(EnoRiver_Components,
                      Observed = EnoRiverDischargeNoGap$Discharge,     
                      Date = EnoRiverDischargeNoGap$Date) ##add in date so we can look at these over time
```
>Observed=discharge
...
```
>clear creek fed snowmelt 
> Seasonal: There is visible seasonality in both the Eno River and Clear Creek datasets. While the Clear Creek data features one large peak around the springtime of each year (presumably the effect of snowmelt entering the river and increasing discharge), the Eno River data actually has two seasonal peaks. There is a peak of larger magnitude also around the springtime of each year, but there is a peak of smaller magnitude in between each larger peak, presumably around the wintertime.
...
> According to the Mann-Kendall test, we fail to reject the null hypothesis that there is no monotonic trend in the Clear Creek discharge over time (Mann-Kendall test, Z=4.39, p=0.09719). Because the resultant p-value is >0.05, at a 5% level of significance, we can't conclude that there is a monotonic trend in Clear Creek discharge over time. 
...
> According to my summary of the seasonal Mann-Kendall test, we reject the null hypothesis that there is no monotonic trend in the Clear Creek discharge over time, and therefore there is a monotonic trend for both March and April. It appears that Season 4, or the month of April, has a resultant p value=0025081, which is <0.05. 
